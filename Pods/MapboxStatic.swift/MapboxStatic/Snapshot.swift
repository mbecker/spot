...
//                    let formattedCount = NumberFormatter.localizedString(from: NSNumber(maximumCountOfRequests), number: .decimal)
                    
                    var formatter: NumberFormatter = NumberFormatter()
                    formatter.numberStyle = .decimal;
                    var formattedStr: NSString = formatter.string(from: NSNumber(value: Int32(maximumCountOfRequests)))! as NSString
                    
                    failureReason = "More than \(formattedStr) requests have been made with this access token within a period of \(formattedInterval)."
                }
